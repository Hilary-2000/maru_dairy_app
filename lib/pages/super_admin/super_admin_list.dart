...
class SuperAdminList extends StatefulWidget {
  const SuperAdminList({super.key});

  @override
  State<SuperAdminList> createState() => _SuperAdminListState();
}
...
class _SuperAdminListState extends State<SuperAdminList> {
  ...
  bool loading = false;
  bool _init = false;
  var super_administrator_list = [];
  var display_list = [];
  List<Color> colors_shade = [];
  List<TextStyle> textStyles = [];
  TextEditingController searchSuperAdmin = TextEditingController();

  Future<void> getSuperAdministrators() async {
    setState(() {
      loading = true;
    });
    ApiConnection apiConnection = new ApiConnection();
    var response = await apiConnection.displaySuperAdmin();
  ...
  void findKeyWord(keyword){
    var newHistory = [];
    for(var item in super_administrator_list){
      int present = 0;
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: Text("Our Super Administrators", style: customs.secondaryTextStyle(size: 20, fontweight: FontWeight.bold),),
                  ),
                  Container(
                    ...
                    child: Material(
                      child: Container(
                        ...
                        child: customs.maruSearchTextField(
                            ...
                            editingController: searchSuperAdmin,
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      await Navigator.pushNamed(
                                          context, "/super_admin_details", arguments: {"index" : index , "super_admin_id": item['user_id']});
                                      getSuperAdministrators();
                                    },
                                    child: Container(
                            ...
                            children: [
                              Container(
                                ...
                                child: Column(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: [
                                    Text("No Super Administrators Found!", style: customs.primaryTextStyle(size: 20, fontweight: FontWeight.bold),),
                                    Spacer(),
                                    SizedBox(
                                      width: width,
                                      child: Image(
                                    ...
                                    Container(
                                      ...
                                      child: CircleAvatar(
                                        ...
                                        child: IconButton(
                                          onPressed: () async {
                                            await Navigator.pushNamed(context, "/new_super_admin");
                                            getSuperAdministrators();
                                          },
